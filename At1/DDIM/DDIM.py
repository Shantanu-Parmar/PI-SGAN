...
output_dir = f"outputs/DDIM_{dataset_name}"
logs_dir = f"logs/DDIM_{dataset_name}"
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"{logs_dir}/ddim_training.log"),
...
class DDIM(nn.Module):
    ...
    def p_sample(self, x, t, t_index):
        ...
        sigma = torch.zeros_like(sqrt_one_minus_alphas_cumprod_t)  # DDIM assumes deterministic sampling
        if t_index == 0:
            return (x - sqrt_one_minus_alphas_cumprod_t * predicted_noise) / sqrt_alphas_cumprod_t
        t_prev = torch.clamp(t - (self.timesteps // self.sampling_steps), min=0)
        sqrt_alphas_cumprod_t_prev = self.extract(self.sqrt_alphas_cumprod, t_prev, x.shape)
        sqrt_one_minus_alphas_cumprod_t_prev = self.extract(self.sqrt_one_minus_alphas_cumprod, t_prev, x.shape)
        pred_x0 = (x - sqrt_one_minus_alphas_cumprod_t * predicted_noise) / sqrt_alphas_cumprod_t
        x_prev = sqrt_alphas_cumprod_t_prev * pred_x0 + sqrt_one_minus_alphas_cumprod_t_prev * predicted_noise
        return x_prev + sigma * torch.randn_like(x)
...
if __name__ == '__main__':
    ...
    logger.info("Initializing U-Net and DDIM...")
    unet = UNet(channels=channels).to(device)
    ddim = DDIM(unet, timesteps=timesteps, sampling_steps=sampling_steps).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for i, real_images in enumerate(dataloader):
            ...
            with autocast('cuda'):
                ...
                noisy_images = ddim.q_sample(real_images, t, noise)
                predicted_noise = ddim(noisy_images, t)
                loss = F.mse_loss(predicted_noise, noise)
            
            scaler.scale(loss).backward()
            torch.nn.utils.clip_grad_norm_(ddim.parameters(), max_norm=1.0)
        ...
        # Save checkpoints
        checkpoint_path = f"{output_dir}/ddim_checkpoint_epoch_{epoch}.pth"
        ...
        if (epoch + 1) % 5 == 0:
            with torch.no_grad():
                ...
                fake_images = ddim.sample((num_eval_samples, channels, *image_size))
