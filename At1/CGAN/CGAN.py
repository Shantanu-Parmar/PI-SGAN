...
output_dir = f"outputs/CGAN_{dataset_name}"
logs_dir = f"logs/CGAN_{dataset_name}"
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"{logs_dir}/cgan_training.log"),
...
class AstroDataset(Dataset):
    def __init__(self, image_dir, transform=None, use_labels=True):
        ...
        # Assume labels are in a file or inferred from subdirectories
        self.labels = self._load_labels()
        logger.info(f"Loaded {len(self.images)} images and {len(self.labels)} labels from {image_dir}")
