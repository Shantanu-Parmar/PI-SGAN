...
class CMRA_GAN:
    ...
    # def infer(self, mobil_image, ref_image, obj_name, output_dir="inferences"):
    #     os.makedirs(output_dir, exist_ok=True)
    #     self.generators[0].eval()
    #     with torch.no_grad():
    #         mobil_tensor = mobil_image.unsqueeze(0).to(device)
    #         ref_tensor = ref_image.unsqueeze(0).to(device)
    #         enhanced_image = self.generators[0](mobil_tensor).cpu()
    #         enhanced_image = (enhanced_image + 1) / 2  # Denormalize from [-1, 1] to [0, 1]
    #         ref_image = (ref_tensor + 1) / 2  # Denormalize for consistency
    #         mobil_image = (mobil_tensor.cpu() + 1) / 2  # Denormalize for consistency

    #         # Create side-by-side grid
    #         grid = utils.make_grid([ref_image[0], mobil_image[0], enhanced_image[0]], nrow=3, normalize=True)
    #         utils.save_image(grid, os.path.join(output_dir, f'{obj_name}_inference_{os.urandom(8).hex()}.png'))
    #         logger.info(f"Saved inference image for {obj_name} to {output_dir}")


    def infer(self, mobil_image, ref_image, obj_name, epoch):
        self.generators[0].eval()
        with torch.no_grad():
            mobil_image = mobil_image.unsqueeze(0).to(self.device)
            ref_image = ref_image.unsqueeze(0).to(self.device)
            enhanced_image = self.generators[0](mobil_image)

            # Bring all to CPU for saving / visualization
            mobil_image = mobil_image.cpu()
            ref_image = ref_image.cpu()
            enhanced_image = enhanced_image.cpu()

            grid = utils.make_grid(
                [ref_image[0], mobil_image[0], enhanced_image[0]],
                nrow=3, normalize=True
            )
            save_path = f"validation_images/{obj_name}_epoch{epoch}.png"
            os.makedirs("validation_images", exist_ok=True)
            save_image(grid, save_path)
            logger.info(f"Saved validation image for {obj_name} at epoch {epoch}")
    ...
    #     ref_images = (ref_batch + 1) / 2
    #     enhanced_images = (enhanced_batch + 1) / 2
    ...
    def validate(self, dataloader, epoch):
        ...
        with torch.no_grad():
            for i, (mobil_batch, ref_batch, obj_name) in enumerate(dataloader):
                ...
                # enhanced_batch = self.generators[0](mobil_batch).cpu()
                # enhanced_batch = (enhanced_batch + 1) / 2
                # ref_batch = (ref_batch + 1) / 2
                # mobil_batch = (mobil_batch.cpu() + 1) / 2
                enhanced_batch = self.generators[0](mobil_batch)  # keep on GPU
                ref_batch = (ref_batch + 1) / 2
                mobil_batch = (mobil_batch + 1) / 2
                enhanced_batch = (enhanced_batch + 1) / 2
                ...
                lpips_val = self.calculate_lpips(ref_batch, enhanced_batch)
                confidence = self.discriminators[0](enhanced_batch.to(device)).mean().item()
        ...
        metrics_df.to_csv("validation_metrics.csv", mode='a', index=False)
        logger.info(f"Validation metrics saved to validation_metrics.csv for epoch {epoch}")
    ...
    def train(self, epochs=50):
        ...
        for epoch in range(epochs):
            ...
            if "test" in dataloaders:
                with torch.no_grad():
                    test_mobil, test_ref, test_obj = next(iter(dataloaders["test"]))
                    test_mobil, test_ref = test_mobil.to(device), test_ref.to(device)
                    self.infer(test_mobil[0].cpu(), test_ref[0].cpu(), test_obj[0], epoch)
