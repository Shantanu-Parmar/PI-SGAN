...
class AstroDataset(Dataset):
    def __init__(self, mobil_dir, ref_dir, split, transform=None):
        ...
        for obj in self.object_names:
            mobil_count = len(self.mobil_images.get(obj, []))
            ref_count = len(self.ref_images.get(obj, []))
            logger.info(f"Object {obj}: {mobil_count} Mobil images, {ref_count} Reference images")

        # Use only objects with both Mobil and Reference images (exclude Jupiter)
        self.common_objects = self.object_names.intersection(self.mobil_images.keys(), self.ref_images.keys()) - {'Jupiter'}

        if not self.common_objects:
            logger.warning(f"No objects with both Mobil and Reference images found in {self.mobil_dir} or {self.ref_dir}")

    def __len__(self):
        if not self.common_objects:
            return 0
        # Return total number of image pairs across all objects
        return sum(min(len(self.mobil_images[obj]), len(self.ref_images[obj])) for obj in self.common_objects)

    def __getitem__(self, idx):
        if not self.common_objects:
            raise IndexError("No valid image pairs available")

        # Find the object and index for this idx
        cumulative_size = 0
        for obj in self.common_objects:
            obj_size = min(len(self.mobil_images[obj]), len(self.ref_images[obj]))
            if idx < cumulative_size + obj_size:
                obj_idx = idx - cumulative_size
                mobil_img = Image.open(self.mobil_images[obj][obj_idx % len(self.mobil_images[obj])]).convert("RGB")
                ref_img = Image.open(self.ref_images[obj][obj_idx % len(self.ref_images[obj])]).convert("RGB")
                if self.transform:
                    mobil_img = self.transform(mobil_img)
                    ref_img = self.transform(ref_img)
                return mobil_img, ref_img, obj
...
if __name__ == "__main__":
    ...
    ref_dir = "MobilTelesco_Processed/augmented_ground_truths"
    
    gan = CMRA_GAN(mobil_dir, ref_dir)
    checkpoint_path = "checkpoints/checkpoint_epoch_35.pth"
    ...
    # Adjust epochs based on checkpoint (assuming checkpoint contains the last epoch)
    start_epoch = 0  # Default if no checkpoint
    if os.path.exists(checkpoint_path):
        checkpoint = torch.load(checkpoint_path)
        start_epoch = checkpoint['epoch']
        logger.info(f"Starting from epoch {start_epoch + 1}")
    total_epochs = 100  # Total desired epochs
    remaining_epochs = max(0, total_epochs - start_epoch)
    if remaining_epochs > 0:
        gan.train(epochs=remaining_epochs)
    else:
        logger.info("Training already completed up to or beyond the target epoch count")