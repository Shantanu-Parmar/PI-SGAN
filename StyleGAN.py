...
output_dir = f"outputs/StyleGAN_{dataset_name}"
logs_dir = f"logs/StyleGAN_{dataset_name}"
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"{logs_dir}/stylegan_training.log"),
...
class AstroDataset(Dataset):
    ...
    def _load_labels(self):
        # Placeholder: Not used for StyleGAN
        return None
...
class Generator(nn.Module):
    ...
    def forward(self, w):
        out = self.base(w.view(-1, latent_dim, 1, 1))
...
if __name__ == '__main__':
    ...
    logger.info("Initializing mapping network, generator, and discriminator...")
    mapping = MappingNetwork(latent_dim).to(device)
    generator = Generator(latent_dim, out_size=image_size).to(device)
    discriminator = Discriminator().to(device)
    criterion = nn.BCEWithLogitsLoss()
    optimizer_G = optim.Adam(list(mapping.parameters()) + list(generator.parameters()), lr=lr, betas=(beta1, 0.999))
    ...
    for epoch in range(num_epochs):
        ...
        for i, real_images in enumerate(dataloader):
            ...
            torch.nn.utils.clip_grad_norm_(list(mapping.parameters()) + list(generator.parameters()), max_norm=1.0)
        ...
        # Save checkpoints
        checkpoint_m_path = f"{output_dir}/mapping_checkpoint_epoch_{epoch}.pth"
        ...
        torch.save(mapping.state_dict(), checkpoint_m_path)
        torch.save(generator.state_dict(), checkpoint_g_path)
        torch.save(discriminator.state_dict(), checkpoint_d_path)
        logger.info(f"Saved checkpoints: {checkpoint_m_path}, {checkpoint_g_path}, {checkpoint_d_path}")

        # Inference and Metrics (every 5 epochs)
        if (epoch + 1) % 5 == 0:
            with torch.no_grad():
                num_eval_samples = min(2, batch_size_actual)
                w = mapping(torch.randn(num_eval_samples, latent_dim).to(device))
                fake_images = generator(w)
