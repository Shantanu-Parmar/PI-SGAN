...
output_dir = f"outputs/BigGAN_{dataset_name}"
logs_dir = f"logs/BigGAN_{dataset_name}"
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"{logs_dir}/biggan_training.log"),
...
class AstroDataset(Dataset):
    def __init__(self, image_dir, transform=None, use_labels=True):
        self.image_dir = image_dir
        self.transform = transform
        self.use_labels = use_labels
        exts = ('.jpg', '.jpeg', '.png')
        self.images = sorted([os.path.join(image_dir, img) for img in os.listdir(image_dir) if img.lower().endswith(exts)])
        self.labels = self._load_labels() if use_labels else None
        logger.info(f"Loaded {len(self.images)} images and {len(self.labels) if use_labels else 0} labels from {image_dir}")

    def _load_labels(self):
        # Placeholder: Assign random labels (0-7) for 8 classes
        # Replace with actual label loading (e.g., from labels.txt or folder structure)
        labels = []
...
class Generator(nn.Module):
    def __init__(self, latent_dim=100, num_classes=8, out_size=(1536, 2048)):
        super(Generator, self).__init__()
        self.latent_dim = latent_dim
        self.label_emb = nn.Embedding(num_classes, latent_dim)
        self.base = nn.Sequential(
            nn.ConvTranspose2d(latent_dim * 2, 512, 8, 1, 0, bias=False),  # 1 -> 8
            nn.BatchNorm2d(512),
            nn.ReLU(True),
    ...
    def forward(self, noise, labels):
        label_emb = self.label_emb(labels).view(-1, self.latent_dim, 1, 1)
        gen_input = torch.cat((noise, label_emb), dim=1)
        out = self.base(gen_input)
...
class Discriminator(nn.Module):
    def __init__(self, num_classes=8):
        ...
        self.label_emb = nn.Embedding(num_classes, 1536 * 2048)
        self.main = nn.Sequential(
            nn.Conv2d(4, 32, 4, 2, 1, bias=False),   # 1536 -> 768
    ...
    def forward(self, x, labels):
        label_emb = self.label_emb(labels).view(-1, 1, 1536, 2048)
...
if __name__ == '__main__':
    ...
    dataset = AstroDataset(image_dir, transform=transform, use_labels=True)
    ...
    generator = Generator(latent_dim, num_classes, out_size=image_size).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (real_images, labels) in enumerate(dataloader):
            ...
            logger.info(f"Batch {i}/{len(dataloader)}, Actual batch size: {batch_size_actual}, Image shape: {real_images.shape}, Labels shape: {labels.shape}")
            
            real_label = torch.full((batch_size_actual, 1), 0.8, device=device)
            fake_label = torch.full((batch_size_actual, 1), 0.2, device=device)
        ...
        if (epoch + 1) % 5 == 0:
            with torch.no_grad():
                ...
                fake_labels = torch.randint(0, num_classes, (num_eval_samples,), device=device)
                fake_images = generator(torch.randn(num_eval_samples, latent_dim, 1, 1).to(device), fake_labels)
                fake_images_display = (fake_images + 1) / 2
                grid = torchvision.utils.make_grid(fake_images_display.cpu(), nrow=1)
                image_path = f"{output_dir}/fake_images_epoch_{epoch+1}_class{fake_labels[0].item()}.png"
